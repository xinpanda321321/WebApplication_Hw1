...
const Product = class Product {
    ...
    constructor(data) {
        var temp = data.split(',');
        this.name = temp[0];
        this.price = parseFloat(temp[1]);
        this.availability = temp[2];
    }
    ...
    static inStock = (products) => {
        ...
        for(const temp of products) {
            if(temp.availability.split(' ')[0] == "In") {
    ...
    static halfOff = (products) => {
        var reducedProducts = new Array();
        for(const temp of products) {
            const p = new Product(temp.name + "," + temp.price * 0.5 + "," + temp.availability);
    ...
    static printProducts = (products) => {
        for(const temp of products) {
            if(temp.availability.split(' ')[0] == "In") {
                console.log("Product: " + temp.name + ", Cost: $" + (Math.round(temp.price * 100) / 100).toFixed(2) + ", Availability: Yes");
            }
            else {
                console.log("Product: " + temp.name + ", Cost: $" + (Math.round(temp.price * 100) / 100).toFixed(2) + ", Availability: No");
            }
        }
    }
